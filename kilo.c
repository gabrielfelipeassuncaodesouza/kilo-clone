...
char *C_HL_keywords[] = {
"switch", "if", "else", "while", "for", "default", "#include",
"break", "continue", "return", "struct", "#define", "#ifdef", 
"union", "typedef", "enum", "class", "case", "#ifndef",
"#endif",
...
#define TOP_LFT_CRNR_CURSR "\x1b[H"

/*** terminal ***/

void die(const char* s) {

	write(STDOUT_FILENO, CLR_SCREEN, 4); //clear scrern
	write(STDOUT_FILENO, TOP_LFT_CRNR_CURSR, 3); //set cursor position
...
//and disable more ctrl signals
	raw.c_oflag &= ~(OPOST); //turn off post processing
	raw.c_cflag |= (CS8);
	raw.c_lflag &= ~(ECHO | ICANON | IEXTEN | ISIG); //turn of echoing,
//Ctrl signals and we can read by byte

	raw.c_cc[VMIN] = 0;
	raw.c_cc[VTIME] = 1;
//lines above mades a time for read funcion
	...
	if(fd != -1) {
		if(ftruncate(fd, len) != -1) {
			if(write(fd, buf, len) != -1) {
				...
				free(buf);
				E.dirty = 0;
				editorSetStatusMessage("%d bytes saved on disk", len);
				return;
			}
		}
...
void editorScroll() {
	...
	if(E.cursory >= E.rowoff + E.screen_height) {

		E.rowoff = E.cursory - E.screen_height + 1;
	}
	...
	if(E.rx >= E.coloff + E.screen_width) {

		E.coloff = E.rx - E.screen_width + 1;
	}
}
...
void editorDrawRows(struct abuf *ab) {
	...
	int curr_row;
	for(curr_row = 0; curr_row < E.screen_height; curr_row++) {

		/*** convert types  ***/

		char bufnum[((sizeof(curr_row) * CHAR_BIT) + 2)/3 + 2];
		sprintf(bufnum, "%d", curr_row);
		char *num = bufnum;

		int llen = 3;
		int lineoffset = 2;

		if(curr_row >= 10 && curr_row < 100) lineoffset = 1;
		if(curr_row >= 100) lineoffset = 0;

		/*** draw ***/

		int filerow = curr_row + E.rowoff;

		if(filerow >= E.numrows) {

			if(E.numrows == 0 && curr_row == E.screen_height / 3) {
				...
				if(welcomelen > E.screen_width)
					welcomelen = E.screen_width;
...
void editorDrawMessageBar(struct abuf *ab) {
	...
	if(msglen && time(NULL) - E.statusmsg_time < MESSAGE_TIME)
...
void editorRefreshScreen() {
	...
	abAppend(&ab, TOP_LFT_CRNR_CURSR, 3); //puts the cursor in the top-left corner
	...
	/*** changes the cursor position appearance ***/
	char buf[32];
        ...
        switch(c) {
                ...
                case CTRL_KEY('q'):
                        ...
                        if(E.dirty && quit_times > 0) {
                                ...
                                editorSetStatusMessage("WARNING!!! File has unsaved changes, Press Ctrl-Q one more time to confirm");
                                quit_times--;
                                return;
                        }
                ...
                if(c == CTRL_KEY('a')) {
                        ...
                        E.cursory = E.rowoff + E.screen_height - 1;
                ...
                while(times--)
                        editorMoveCursor(c == CTRL_KEY('a')  ? ARROW_UP : ARROW_DOWN);
                }
                break;
